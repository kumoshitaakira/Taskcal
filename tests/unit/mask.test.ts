...
describe("送信前のマスク（RFC-004 §5 / ADR-008）", () => {
  ...
  it("直後に時刻が続いても電話番号を取りこぼさない", () => {
    // 貪欲に拾って桁数で捨てる方式だと、候補が隣の時刻まで伸びて候補ごと
    // 捨てられ、電話番号が素通りする。
    expect(maskContactInfo("080-1234-5678 18から勤務できます").text).toBe(
      "[電話番号] 18から勤務できます",
    );
    expect(maskContactInfo("09012345678 18-22 で").text).toBe("[電話番号] 18-22 で");
    expect(maskContactInfo("03-1234-5678 18から").text).toBe("[電話番号] 18から");
  });

  it("日本の主な区切り方を網羅する", () => {
    for (const phone of [
      "080-1234-5678", // 3-4-4（携帯）
      "03-1234-5678", // 2-4-4（東京）
      "050-1234-5678", // 3-4-4（IP）
      "0120-117-117", // 4-3-3（フリーダイヤル）
      "0570-000-000", // 4-3-3
      "0800-123-4567", // 4-3-4
      "0120-11-7117", // 4-2-4
      "09012345678", // 区切り無し
      "+81-90-1234-5678", // 国際表記
      "(03) 1234-5678", // 括弧付き市外局番
    ]) {
      expect(maskContactInfo(phone).text, phone).toBe("[電話番号]");
    }
  });
