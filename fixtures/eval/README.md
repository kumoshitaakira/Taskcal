...
| A07 | `A07-readback-old-source.json` | 更新後に元CSVを読戻す | 読戻し不一致を完了扱いせず、旧版へ自動復帰しない | 未実行：正式版参照と読戻し照合 |
...
- `UNKNOWN`：結果不明。再実行せず、照合可能なら照合し、不能なら`ATTENTION`／
  `HANDED_OFF`へ採用事実を保持して引き継ぐ。
