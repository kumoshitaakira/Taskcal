...
**ただし `ReconcileRequired` に留め続けない。** 照会が継続不能になったら、停止していても
Q11 の `ReconcileRequired → Attention` へ出す。ここを止めると、停止済みの案件だけが永久に
非終端で残り、Q12 の `Attention → HandedOff`（人が引き取る経路）にも届かない。ADR-022 が
解こうとした永久非終端を、停止済みに限って作り直すことになる。

つまり停止済みの案件の行き先は次の三つに分かれる。

| 照合の結果 | 行き先 |
|---|---|
| 採用済みと確認 | `Committed` 以降（確定事実を保持） |
| 未採用と確認 | 停止理由に応じた終端（`Cancelled` / `HandedOff`） |
| 成否不明・照会はまだ可能 | `ReconcileRequired` のまま待つ |
| 成否不明・照会が継続不能 | `Attention`（人の対応。そこから `HandedOff`） |
