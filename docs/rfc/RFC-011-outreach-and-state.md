...
作成：2026-09-20／状態：打診・承諾方針は現行文書、状態詳細はレビュー反映案  
...
v0.4の処理段階をそのまま案件の唯一の状態にすると、「Aは確認中、Bは承諾済み」を扱いにくい。以下の分離を提案する。
...
この図はQ07の推奨案「必要な通知受付まで業務完了」を採った提案。確定と通知を別完了にするなら指標・画面・状態を対応して変更する。

完了後の訂正は自動調整を再開せず、元のcompletedAt・確定内容を保持した追記の変更申告として扱う提案とする。CaseOutcome一つを上書きして過去の完了を消さない。別Handoffテーブルは必須ではない。
