...
- デモ録画で観測：返信の解釈（OrcaRouter応答待ち、最長60秒）の間に worker の heartbeat が
  止まり、店長画面が「接続できない」と表示した。解釈の直前にも beat し、`WORKER_STALE_MS` を
  30秒から90秒（タイムアウト上限より長く）へ変えた。

### PR #18 のレビュー（Codex・Copilot・nichikaw00）で直したもの

- **操作記録の後勝ち上書き／実行中を「記録なし＝未反映」と読む窓**（P1・高）。CSV管理版ストアの
  操作記録を、成果物を書く前に `IN_PROGRESS` マーカーとして排他的に作る（`wx`）。同じ操作IDの
  並行呼出しは作成に負けた側が既存記録の内容ハッシュを照合し、違えば `CONFLICT`、同じで進行中なら
  `UNKNOWN`。照会は、記録なし→`NOT_APPLIED`（一度も始まっていない）、5分以内の進行中→`UNKNOWN`、
  それより古い進行中→`NOT_APPLIED`（結果は返っておらず採用もされていない）。
- **成功済みの打診開始を、遅れて失敗した並行呼出しの `REFUSED` で上書きし得た**（高）。拒否を書く前に
  操作の状態を読み、確定済みならその結果を再生する。
- **勤務表の読込み中に期限を過ぎても打診を積めた**（P2）。取引Bで時計を取り直して期限を再検査する。
- **別営業日の採用が並行するとデッドロックし得た**（高）。`lockMonth` で同月の参照行を `schedule_id`
  順に先にロックする。**参照を持たない営業日**が同月にあれば `missing` として採用を巻き戻す（中）。
- `loadSchedule` は成果物参照が管理版を指す形でなければ断る（形が違う参照を通していた）。
- ADR-026 に「実装上の仮定であり承認待ち」の位置づけと、承認されない場合の代替を追記。
- 回帰テスト：同一操作IDの並行呼出し、実行中マーカーの照会、成功結果の上書き防止、期限の再検査。

残る指摘（直していない）：無し。fence token 自体は未実装（README「動かないもの」）。
