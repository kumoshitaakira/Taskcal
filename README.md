...
ハッカソン向けの実装中です（Day 2、2026-09-22）。現時点で動くのは**欠勤の登録から、適格候補への同時打診、模擬受信箱への送信、返信の受信と永続化まで**です。返信の解釈はOrcaRouterが未設定のため実行できません。正式採用の進行（選定の固定・作業用成果物の検査・一括採用・読戻し・通知）は実装しましたが、CSV取込と候補選定（担当B）が未実装のため、**本番経路では未実装として断ります**。
...
そのまま再現できている受入ケースは **A11・A12・A15・A18の一部**（予算・回数上限）に加え、
**A02・A03・A04・A05・A08**（`tests/integration/adopt-plan.test.ts`）です。A07・A13 は
一部のみ。A01・A06・A14・A16・A17 は未実行です。

**正式採用のテストは選定とCSVを台（fake）に差し替えています。** `SelectionPlanner`・
`EligibilityChecker`・`ScheduleGateway`（いずれも担当B）の実装が無いためで、Q02の被覆・
重複や月次上限の規則、CSVの往復は動いていません。手順（取引の切り方・再検査・照合・
巻き戻し）を確かめたのであって、選定規則やCSV往復を確かめたのではありません。
テスト名の付け方は [`tests/README.md`](tests/README.md) にあります。設計や fixture が
あることを合格と読まないでください。
