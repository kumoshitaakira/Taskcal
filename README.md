...
受入ケースの状況は**2段に分かれます**。同じ「確認済み」で括らないでください。

| 段 | ケース | 意味 |
|---|---|---|
| 本番経路でそのまま再現 | A11・A12・A15・A18の一部（予算・回数上限） | 合成の根の実装で端から端まで通る |
| **担当Bの口を台（fake）に差し替えて手順だけ確認** | A02・A03・A05・A08、A04とA07とA13の一部 | 取引の切り方・再検査・照合・巻き戻しは確かめた。**選定規則とCSV往復は動いていない** |
