 ...
 * 復旧の再試行間隔（ミリ秒）。
 *
 * 戻らない案件・照会できない通知を worker のtickごとに触ると、外部照会と
 * 追記履歴が際限なく増える（2秒tickなら1日約4万件）。**復旧は進まないことが
 * 普通にある**ので、間隔を空けて他の案件へ順番を回す。
 *
 * 短くしても復旧は早くならない。早くなるのは外部側の事情が変わったときだけで、
 * それは秒単位では起きない。
 */
export const RECOVERY_RETRY_MS = 60_000;
