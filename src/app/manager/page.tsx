...
export default async function ManagerPage({
  ...
  return (
    <main>
      <h1>店長画面</h1>
      <p className="lede">
        欠勤の登録、適格な同職種への同時打診、返信の受信と解釈、承諾の生成、候補選定、CSV管理版の生成・読戻し、正式採用、通知、調整の停止までが動きます。打診の直前と正式採用の直前に在籍・職種・同日の勤務との重複・月次上限を検査します。本人の可能時間表は無く、打診時は必要枠を、採用時は承諾した区間を可能時間として扱うため、可能時間そのものは未検査です。
      </p>
      <nav className="links">
        <Link href="/">トップ</Link>
          ...
          <p className="lede" style={{ margin: 0 }}>
            {resuming && stopped
              ...
              : resuming
                ...
                : "選定可の承諾から、必要枠を各区間ちょうど1人で覆う計画を選び（Q02）、CSV管理版の生成・読戻しを経て正式採用します。採用の直前に案件版・停止・期限・承諾・未処理返信・月内入力の完全性・適格性をもう一度検査します（D08）。"}{" "}
            <strong>
              重なる承諾や隙間の残る承諾しか無い場合は採用せず、調整中のまま据え置きます（A16）。
            </strong>
          </p>
          <button type="submit">
            {resuming && stopped
      ...
      {view.activeCase && view.activeCase.outreaches.length === 0 ? (
        <form action={startOutreachAction} className="panel">
          ...
          <input type="hidden" name="caseId" value={view.activeCase.caseId} />
          <p className="lede" style={{ margin: 0 }}>
            同職種の在籍者（欠勤者本人を除く）のうち、同日の勤務と重ならず月次割当上限に収まる全員へ個別に打診します。外した相手と理由は案件の処理記録（DB）に残ります。この画面には出ません。送信はworkerが行います。
          </p>
