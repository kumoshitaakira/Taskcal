...
- **同じIDで内容が違えば拒否する**（D07）。受信内容のハッシュを保存して照合する。
  IDだけを見て重複と決めると、保存済みの事実と食い違う結果を返してしまう。
...
受信口（`/api/mock-channel/inbound`）は**署名検証も認証も無い**。叩ける者は架空
スタッフの返信を任意に投入できる。MVPは架空データだけを扱い、ローカルのデモでしか
動かさない前提なのでこれで足りる。前提をコードでも守るため、`NODE_ENV=production`
では `MOCK_CHANNEL_ENABLED=1` が無い限り 404 を返す。本物の連絡経路を繋ぐ場合は、
この入口ではなく署名検証と認証境界を持つ別の経路を作る。

**入口が受理する形はDBも受け取る。** 形式の検査を入口より厳しくすると、書き方の違い
だけで受信がロールバックされる（大文字のUUIDで実際に起きた）。保存する値はrepositoryが
小文字へ揃え、比較の揺れを作らない。
