 ...
 * 余地ができる。
 *
 * **前提：1つの案件の呼出しはすべて同じ `run_id` を使う。** `run_id` は
 * `absence_case.run_id` に固定してあり、呼出し元（`interpret-reply.ts`）は案件の値を
 * そのまま渡す。この前提が崩れると、同じ案件を別の run で並行実行したときに
 * `case_call_limit` / `case_spend_limit` の同時判定がすり抜ける。案件をまたいで
 * run を分ける設計にする場合は、ロックのキーを案件側へ移すこと。
 */
