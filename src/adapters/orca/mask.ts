...
const SIMPLE_RULES: {
  ...
  //
  // **終端文字を並べるのではなく、URLとして許す文字を列挙する。**
  // 「この記号でも終端する」を都度足す方式で、日本語句読点・ASCIIカンマ・
  // ASCII閉じ括弧と3回追加することになった。許可文字を列挙すれば、列挙して
  // いない文字はすべて終端になる。
  //
  // 許すのは RFC 3986 の unreserved（`A-Za-z0-9-._~`）と、実際に使う区切り
  // `/:?#@&=+%`。括弧・カンマ・セミコロン・感嘆符・引用符は**含めない**。
  // それらを含むURLは途中で切れるが、URLは元々マスク対象であり、勤務条件を
  // 壊すより切れる側へ倒す。
  // 末尾は記号で終わらせない（文末の `.` などをURLへ取り込まないため）。
  {
    kind: "url",
    pattern: /https?:\/\/[A-Za-z0-9\-._~:/?#@&=+%]*[A-Za-z0-9\-_~/@&=+%]/g,
    token: "[URL]",
  },
];
